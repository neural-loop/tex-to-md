...
def apply_alphabetical_operations(tex_content):
    ...
    # tex_content = re.sub(r' <cit.> ', '', tex_content)
    # tex_content = re.sub(r'<cit.> ', '', tex_content)
    # tex_content = re.sub(r'<cit.>', '', tex_content)

    tex_content = re.sub(r'\s*\[?\s*<cit\.\>\s*\]?', '', tex_content)
    tex_content = re.sub(r'\s*<cit\.', '', tex_content)
...
def post_clean_string_replace(input_path):
    ...
    # tex_content = tex_content.replace(' <cit.>', '')
